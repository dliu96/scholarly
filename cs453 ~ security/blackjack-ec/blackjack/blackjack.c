...
enum { STATE_AUTH, STATE_IDLE, STATE_PLAYING };
...
{
	if (game->state == STATE_AUTH)
	{
		nprintf("-ERR You must login first\n");
		return;
	}
...
void cmd_login(struct game *game, int argc, char *argv[])
{
	if (game->state != STATE_AUTH)
	{
		nprintf("-ERR Incorrect state\n");
		return;
	}

	if (!auth_userpass(argv[1], argv[2]))
	{
		nprintf("-ERR Login failed\n");
		return;
	}

	nprintf("+OK Login successful\n");
	game->user = strdup(argv[1]);
	game->state = STATE_IDLE;
}
