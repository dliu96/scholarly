...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for distribution in livingGhostPositionDistributions:
            # set the largest position in our distribution as our best guess to where the ghost will go
            bestGuess = distribution.argMax()

            # use manhattan distance to our distance to the best guess
            distance = util.manhattanDistance(pacmanPosition, bestGuess)

            #first iteration
            if minDistance > distance or starting:
                minDistance = distance
                closestPos = bestGuess
                starting = False

        # find best action
        minDistance = 0

        #iterate through each action individually
        for action in legal:
            newPos = Actions.getSuccessor(pacmanPosition, action)

            #calculate distance at newPos
            distanceAfterAction = util.manhattanDistance(newPos, closestPos)

            #trying to find the shortest path
            if minDistance > distanceAfterAction or minDistance == 0:
                minDistance = distanceAfterAction
                bestAction = action

        #return the single action that gets us the closest
        return bestAction
