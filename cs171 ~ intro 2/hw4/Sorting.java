...
public class Sorting {
	...
	public static void MergeSortNonRec(long[] a) {
		...
		int sub = 1;
		while (sub < N) {
			for (int lo = 0; lo < N - sub; lo += sub * 2) 
				merge(a, aux, lo, lo + sub - 1, Math.min(lo + sub + sub - 1, N - 1));
				sub = sub * 2;
		}
	}// MergeSortNonRec()
	...
	// This tests whether your sorted result is correct by comparing it to
	// reference result
	...
	// This creates an array with n randomly generated elements between (0,
	// n*10]
	...
	public static void main(String[] args) {
		...
		// max defines the maximum size of the array to be tested, which is
		// 2^max
		// runs defines the number of rounds to be performed per test, in order
		// to get an average running time.
		int max = 14, runs = 5;
		double[][] stats = new double[algorithms.length][max];
		for (int i = 0; i < algorithms.length; i++) { // loop through each
														// sorting algorithm
			switch (i) {
			case BUBBLE:
		...
		System.out
				.println("N\t Bubble Sort\t Selection Sort\t Insertion Sort\tMergeSortRec\tMergeSortNon");
